...
class DesktopHelper {
  static const MethodChannel _channel =
      MethodChannel('com.youngfeng.desktop/desktop_helper');
  ...
  static Future<bool> openFile(
      {required String path, bool isDir = true}) async {
    bool isSuccess =
        await _channel.invokeMethod("openFile", {"path": path, "isDir": isDir});
    return isSuccess;
  }

  /// Get applications that can open the file, which path is [path].
  static Future<List<ApplicationInfo>> getAppsForFile(
      {required String path}) async {
    List<dynamic>? apps =
        await _channel.invokeListMethod("getAppsForFile", {"path": path});
    ...
    apps?.forEach((app) {
      ...
      Uint8List icon = Uint8List.fromList(iconData.map((data) {
        log("${data as int}");
        return data as int;
      }).toList());
  ...
  static Future<bool> openFileWithApp(
      {required String filePath, required String appUrl}) async {
    bool isSuccess = await _channel.invokeMethod(
        "openFileWithApp", {"filePath": filePath, "appUrl": appUrl});
    return isSuccess;
  }
}
